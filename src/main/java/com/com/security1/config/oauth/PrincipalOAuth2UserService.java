package com.com.security1.config.oauth;
...
public class PrincipalOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 구글 로그인 버튼을 클릭 -> 구글 로그인  -> 로그인 완료 -> code 리턴(OAuth Client 라이브러리) -> AccessToken 요청
        // userRequest 정보 -> loadUser 메서드 호출 -> 구글로부터 회원 프로필을 받아준다
        System.out.println("getAttributes : " + oAuth2User.getAttributes());

        // 회원을 강제로 진행
        String provider = userRequest.getClientRegistration().getClientId();  // google
        String providerId = oAuth2User.getAttribute("sub");
        String username = provider + "_" + providerId;  // google_115208618508465384182
